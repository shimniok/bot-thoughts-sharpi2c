...
void USI_UART_Initialise_Transmitter( void )
{
	...
	TCCR0B = TIMER_PRESCALER_BITS;							  // Reset the prescaler and start Timer0.
	//Tiny85
...
void USI_UART_Initialise_Receiver( void )
{
    ...
    GIMSK |=  (1<<PCIE0);                                   // Enable pin change interrupt for PORTB
...
ISR(PCINT0_vect)
{
    ...
    {                                                             //  was this pin that generated the pin change interrupt.
        ...
        GIMSK &=  ~(1<<PCIE0);                                    // Disable pin change interrupt for PORTB
        //Tiny85
